    ...
    <div className={`animal-card ${status}`}>
      ...
      <div className="animal-card-description">
        ...
        <div className="hungerstatus">
          ...
          <p>Senast {animal.name} fick mat var {new Date(animal.lastFed).toLocaleString()}.
        </p>
        <button onClick={onFeed} disabled={!feedable}>
          {feedable ? `Mata ${animal.name}` : `${animal.name} är inte hungrig än`}
        </button>
        </div>

        <p>
          <strong>Latinskt namn: </strong><p>{animal.latinName} <br />
        </p></p>
        ...
        <p><strong>Artbeskrivning: </strong></p><p>{animal.longDescription}</p>
